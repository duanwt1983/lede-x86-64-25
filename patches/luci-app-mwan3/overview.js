...
function renderInterfaces(interfaces) {
	...
	return Object.keys(interfaces).map(function(iface) {
		...
		if (si.duration != null)
			children.push(E('div', {}, [
				E('strong', {}, si.durationLabel + ': '),
				format.formatDuration(si.duration)
			]));
		...
		Object.keys(fam).forEach(function(pname) {
			...
			var members = fam[pname].map(function(m) {
				...
				if (m.percent > 0)
					...
					severity = 'muted';
				return E('div', {
					'class': 'mwan3-nowrap ' + components.textClass(severity)
...
function renderRuleRows(rules) {
	...
	(rules || []).forEach(function(r) {
		...
		if (r.proto && r.proto !== 'all')
			match.push('proto: ' + r.proto);
		if (src)
			match.push('src: ' + src);
		if (dst)
			match.push('dst: ' + dst);
		if (r.fwmark && r.fwmask)
			match.push('mark: ' + r.fwmark + '/' + r.fwmask);
		if (r.sticky === '1')
			match.push(_('sticky'));
		/* ui.Table.deriveSortKey() calls hasAttribute on cell nodes.
		   components.text() is a Text node and crashes the Status page. */
		rows.push([
			r['.name'] || '',
