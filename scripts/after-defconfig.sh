...
select_wanted() {
  ...
  enable_pkg wget-ssl
  enable_pkg tcpdump
  enable_pkg luci-app-tcpdump
  enable_pkg luci-i18n-tcpdump-zh-cn
...
grep -E '^CONFIG_PACKAGE_(luci-nginx|nginx|uhttpd|luci-app-samba4|samba4-server|luci-app-passwall|luci-app-mosdns|mosdns|luci-app-netspeedtest|librespeed-go|luci-app-tcpdump|tcpdump|wireshark|luci-app-istorex|luci-app-quickstart|luci-app-fastnet|luci-app-diskman|luci-i18n-diskman-zh-cn|luci-app-filemanager|luci-app-mwan3|mwan3|parted|blkid|kmod-ixgbe|smartmontools|mdadm|nftables-json)=' .config || true
