...
grep -E '^CONFIG_PACKAGE_(luci-app-samba4|samba4-server|luci-app-passwall|luci-app-mosdns|mosdns|luci-app-istorex|luci-app-fastnet|luci-app-diskman|luci-app-filemanager|luci-app-daed|daed|netdata)=' .config || true
grep -E '^CONFIG_(VMDK_IMAGES|GRUB_EFI_IMAGES|CCACHE)=' .config || true
